defmodule SingyeongWeb.Transport.CowboyWebsocket do
  # Implementation of the WebSocket transport for Cowboy.
  # We copied the implementation from Phoenix and adapted it it to allow for
  # accessing raw WS close codes.
  ...
  def websocket_terminate({:remote, code, _}, _req, {handler, state})
      # when code in 1000..1003 or code in 1005..1011 or code == 1015
      do
    # Singyeong: Pass ws close code to the handler to allow application-level handling
    handler.ws_close(code, state)
    :ok
  end
  ...
  defp handle_reply(req, handler, {:reply, frames, new_state}) when is_list(frames) do
    {:reply, frames, req, {handler, new_state}}
  end
end
